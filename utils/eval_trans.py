...
def evaluation_transformer(out_dir, val_loader, net, trans, logger, writer, nb_iter, best_fid, best_iter, best_div, best_top1, best_top2, best_top3, best_matching, clip_model, eval_wrapper, draw = True, save = True, savegif=False) : 
    ...
    for i in range(1):
        for batch in val_loader:
            word_embeddings, pos_one_hots, clip_text, sent_len, pose, m_length, token, name, key = batch
            ...
            for k in range(bs):
                try:
                    word = word_emb[k:k+1,:,:]
                    index_motion = trans.sample(feat_clip_text[k:k+1,:], word, key[k:k+1,:,:],True)
        ...
        for i in range(1):
            ...
            for k in range(bs):
                try:
                    #index_motion = trans.sample(feat_clip_text[k:k+1], False)
                    word = word_emb[k:k+1,:,:]
                    index_motion = trans.sample(feat_clip_text[k:k+1,:], word,True)
                except:
                    index_motion = torch.ones(1,1).cuda().long()
            ...
            if i == 0:
                ...
                nb_sample += bs
                print('real')
                print(R_precision_real  / nb_sample)
                print('pred')
                print(R_precision / nb_sample)
